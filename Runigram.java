...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0; i < image.length; i++) {
			for (int j = 0; j < image[i].length; j++) {
				flippedImage[i][image[i].length - j -1]=image[i][j];
			}	
		}
		return flippedImage;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				...
				int col = (int)(j * ((double) originalWidth / width));
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		...
		Color blendedPixel = new Color(red, green, blue);
		return blendedPixel;
	}
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		Color[][] fitTarget = null;
		if(source.length != target.length || source[0].length != target[0].length)
			fitTarget = scaled(target, source[0].length, source.length);
		else{
			fitTarget = target;	
		}
		for (int i = 0; i < n; i++) {
			display(blend(source, fitTarget, ((n-i)/n)));
			StdDraw.pause(500);
		}
	}
